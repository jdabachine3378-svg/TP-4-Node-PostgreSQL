...
export const livreController = {
  liste: async (req, res) => {
    const { rows: livres } = await LivreModel.getAll();
    res.render('pages/livres/liste', { title: 'Livres', livres, searchTerm: req.query.q || '' });
  },

  rechercher: async (req, res) => {
    const { rows: livres } = await LivreModel.search(req.query.q || '*');
    res.render('pages/livres/liste', { title: 'Recherche', livres, searchTerm: req.query.q });
  },

  ajouterForm: async (_, res) => {
